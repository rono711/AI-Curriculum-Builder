import json
from pathlib import Path

from batch_engine.batch_file_builder import BatchFileBuilder
from batch_engine.config import STAGE2_PROMPT_TYPES


class Stage2BatchBuilder:

    def __init__(self):
        self.batch_builder = BatchFileBuilder()

    @staticmethod
    def make_custom_id(
            request_id,
            lesson_package_id,
            prompt_type
    ):
        return (
            f"{request_id}__"
            f"{lesson_package_id}__"
            f"{str(prompt_type).upper()}"
        )

    def build(
            self,
            request_id,
            prompt_results,
            output_root="data/batches"
    ):
        request_dir = (
            Path(output_root)
            / str(request_id)
        )

        request_dir.mkdir(
            parents=True,
            exist_ok=True
        )

        batch_requests = []
        manifest_entries = []

        for result in prompt_results:

            prompt_type = str(
                result["prompt_type"]
            ).strip().upper()

            if prompt_type not in STAGE2_PROMPT_TYPES:
                continue

            lesson_package_id = str(
                result["lesson_package_id"]
            ).strip()

            prompt = str(
                result["prompt"]
            )

            if not prompt.strip():
                raise RuntimeError(
                    "Empty Stage 2 prompt: "
                    + prompt_type
                )

            custom_id = self.make_custom_id(
                request_id,
                lesson_package_id,
                prompt_type
            )

            batch_requests.append(
                self.batch_builder.make_request(
                    custom_id=custom_id,
                    prompt=prompt
                )
            )

            manifest_entries.append({
                "custom_id":
                    custom_id,
                "request_id":
                    str(request_id),
                "lesson_package_id":
                    lesson_package_id,
                "prompt_type":
                    prompt_type,
                "source_prompt_type":
                    result.get(
                        "source_prompt_type",
                        prompt_type
                    ),
                "prompt_file":
                    result.get(
                        "prompt_file",
                        ""
                    ),
                "metadata_file":
                    result.get(
                        "metadata_file",
                        ""
                    ),
                "workbook_path":
                    result.get(
                        "workbook_path",
                        ""
                    ),
                "curriculum_code":
                    result.get(
                        "curriculum_code",
                        ""
                    ),
            })

        if not batch_requests:
            raise RuntimeError(
                "No Stage 2 prompts were supplied."
            )

        input_path = (
            request_dir
            / "stage2_input.jsonl"
        )

        manifest_path = (
            request_dir
            / "stage2_manifest.json"
        )

        self.batch_builder.write_jsonl(
            batch_requests,
            input_path
        )

        manifest = {
            "request_id":
                str(request_id),
            "stage":
                2,
            "request_count":
                len(batch_requests),
            "input_file":
                str(input_path),
            "entries":
                manifest_entries,
        }

        manifest_path.write_text(
            json.dumps(
                manifest,
                indent=2,
                ensure_ascii=False
            ),
            encoding="utf-8"
        )

        return {
            "request_id":
                str(request_id),
            "stage":
                2,
            "request_count":
                len(batch_requests),
            "input_file":
                str(input_path),
            "manifest_file":
                str(manifest_path),
        }
